...
// Use this on iOS7 when you add an instance of RevealViewController as a child of a UINavigationController (or another SlideViewController)
...
#pragma mark - SlideViewControllerDelegate Protocol
typedef enum
{
    SlideControllerOperationReplaceRearController,
    SlideControllerOperationReplaceFrontController,
    SlideControllerOperationReplaceRightController,
    
} SlideControllerOperation;
...
- (void)revealController:(SlideViewController *)revealController willMoveToPosition:(FrontViewPosition)position;
- (void)revealController:(SlideViewController *)revealController didMoveToPosition:(FrontViewPosition)position;

// This will be called inside the reveal animation, thus you can use it to place your own code that will be animated in sync
- (void)revealController:(SlideViewController *)revealController animateToPosition:(FrontViewPosition)position;

// Implement this to return NO when you want the pan gesture recognizer to be ignored
- (BOOL)revealControllerPanGestureShouldBegin:(SlideViewController *)revealController;

// Implement this to return NO when you want the tap gesture recognizer to be ignored
- (BOOL)revealControllerTapGestureShouldBegin:(SlideViewController *)revealController;

// Implement this to return YES if you want this gesture recognizer to share touch events with the pan gesture
- (BOOL)revealController:(SlideViewController *)revealController
...
- (void)revealControllerPanGestureBegan:(SlideViewController *)revealController;
- (void)revealControllerPanGestureEnded:(SlideViewController *)revealController;
...
- (void)revealController:(SlideViewController *)revealController panGestureBeganFromLocation:(CGFloat)location progress:(CGFloat)progress;
- (void)revealController:(SlideViewController *)revealController panGestureMovedToLocation:(CGFloat)location progress:(CGFloat)progress;
- (void)revealController:(SlideViewController *)revealController panGestureEndedToLocation:(CGFloat)location progress:(CGFloat)progress;

// Notification of child controller replacement
- (void)revealController:(SlideViewController *)revealController willAddViewController:(UIViewController *)viewController
    forOperation:(SlideControllerOperation)operation animated:(BOOL)animated;
- (void)revealController:(SlideViewController *)revealController didAddViewController:(UIViewController *)viewController
    forOperation:(SlideControllerOperation)operation animated:(BOOL)animated;

// Support for custom transition animations while replacing child controllers
- (id<UIViewControllerAnimatedTransitioning>)revealController:(SlideViewController *)revealController
    animationControllerForOperation:(SlideControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC;
...
// We add a category of UIViewController to let childViewControllers easily access their parent SlideViewController
...
extern NSString* const SlideSegueRearIdentifier;
//extern NSString* const SlideSegueFrontIdentifier;
extern NSString* const SlideSegueRightIdentifier;


// This will allow the class to be defined on a storyboard
#pragma mark - SlideViewControllerSegueSetController Classes

// Use this along with one of the segue identifiers to segue to the initial state
@interface SlideViewControllerSegueSetController : UIStoryboardSegue
@end


#pragma mark - SlideViewControllerSeguePushController Classes

// Use this to push a view controller
@interface SlideViewControllerSeguePushController : UIStoryboardSegue
@end


#pragma mark - SlideViewControllerSegue (Deprecated)

@interface SlideViewControllerSegue : UIStoryboardSegue     // DEPRECATED: USE SlideViewControllerSegueSetController instead
@property (nonatomic, strong) void(^performBlock)( SlideViewControllerSegue* segue, UIViewController* svc, UIViewController* dvc );
@end