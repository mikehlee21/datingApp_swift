...
NSString * SlideSegueFrontIdentifier = @"slide_front";
...
#pragma mark - SlideView Class

@interface SlideView: UIView
{
    __weak SlideViewController *_c;
}
...
- (id)initWithFrame:(CGRect)frame controller:(SlideViewController*)controller
{
...
{
    __weak SlideViewController *_revealVC;
    UIView *_view;
...
- (id)initWithRevealController:(SlideViewController*)revealVC containerView:(UIView*)view fromVC:(UIViewController*)fromVC
...
#pragma mark - SlideViewController Class

@interface SlideViewController()<UIGestureRecognizerDelegate>
{
    SlideView *_contentView;
...
- (void)loadView
{
    ...
    _contentView = [[SlideView alloc] initWithFrame:frame controller:self];
...
- (void)_dispatchSetFrontViewPosition:(FrontViewPosition)frontViewPosition animated:(BOOL)animated
{
    ...
    __weak SlideViewController *theSelf = self;
...
{
    [self _performTransitionToViewController:newRearViewController operation:SlideControllerOperationReplaceRearController animated:animated];
}
...
{
    [self _performTransitionToViewController:newFrontViewController operation:SlideControllerOperationReplaceFrontController animated:animated];
}
...
{
    [self _performTransitionToViewController:newRightViewController operation:SlideControllerOperationReplaceRightController animated:animated];
}


// Primitive method for animated controller transition
- (void)_performTransitionToViewController:(UIViewController*)new operation:(SlideControllerOperation)operation animated:(BOOL)animated
{
    ...
    if ( operation == SlideControllerOperationReplaceRearController )
        old = _rearViewController, _rearViewController = new, view = _contentView.rearView;
    
    else if ( operation == SlideControllerOperationReplaceFrontController )
        old = _frontViewController, _frontViewController = new, view = _contentView.frontView;
    
    else if ( operation == SlideControllerOperationReplaceRightController )
...
- (void)prepareForSegue:(SlideViewControllerSegue *)segue sender:(id)sender   // TO REMOVE: DEPRECATED IMPLEMENTATION
{
    // This method is required for compatibility with SlideViewControllerSegue, now deprecated.
    // It can be simply removed when using SlideViewControllerSegueSetController and SlideViewControlerSeguePushController
    
    NSString *identifier = segue.identifier;
    if ( [segue isKindOfClass:[SlideViewControllerSegue class]] && sender == nil )
    {
        if ( [identifier isEqualToString:SlideSegueRearIdentifier] )
        {
        ...
        else if ( [identifier isEqualToString:SlideSegueFrontIdentifier] )
        {
            segue.performBlock = ^(SlideViewControllerSegue* rvc_segue, UIViewController* svc, UIViewController* dvc)
            {
        ...
        else if ( [identifier isEqualToString:SlideSegueRightIdentifier] )
        {
            segue.performBlock = ^(SlideViewControllerSegue* rvc_segue, UIViewController* svc, UIViewController* dvc)
            {
...
- (void)loadStoryboardControllers
{
    if ( self.storyboard && _rearViewController == nil )
    {
        ...
        @try
        {
            [self performSegueWithIdentifier:SlideSegueRearIdentifier sender:nil];
        }
...
//            PFUser *user = [PFUser currentUser];
            ...
            NSString *lastCheckin = [userDefaults objectForKey:@"cityCheckIn"];
            if(lastCheckin != nil){
                PFUser *currentUser = [PFUser currentUser];
                if(currentUser[@"LastCheckIn"] != nil){
                    if([SlideSegueFrontIdentifier isEqualToString:@"slide_front"]){
                        SlideSegueFrontIdentifier = @"slide_front1";
                    }
                }
            }
            [self performSegueWithIdentifier:SlideSegueFrontIdentifier sender:nil];
        }
        @catch(NSException *exception) {}
        
        @try
        {
            [self performSegueWithIdentifier:SlideSegueRightIdentifier sender:nil];
        }
        @catch(NSException *exception) {}
    }
}
...
#pragma mark - UIViewController(SlideViewController) Category

@implementation UIViewController(SlideViewController)

- (SlideViewController*)revealViewController
{
    UIViewController *parent = self;
    Class revealClass = [SlideViewController class];
...
#pragma mark - SlideViewControllerSegueSetController segue identifiers

NSString * const SlideSegueRearIdentifier = @"slide_rear";

NSString * const SlideSegueRightIdentifier = @"slide_right";


#pragma mark - SlideViewControllerSegueSetController class

@implementation SlideViewControllerSegueSetController

- (void)perform
{
    NSString *identifier = self.identifier;
    SlideViewController *rvc = self.sourceViewController;
    UIViewController *dvc = self.destinationViewController;
    
    if ( [identifier isEqualToString:SlideSegueFrontIdentifier] )
        [rvc _setFrontViewController:dvc animated:NO];
    
    else if ( [identifier isEqualToString:SlideSegueRearIdentifier] )
...
#pragma mark - SlideViewControllerSeguePushController class

@implementation SlideViewControllerSeguePushController

- (void)perform
{
    SlideViewController *rvc = [self.sourceViewController revealViewController];
    UIViewController *dvc = self.destinationViewController;
...
#pragma mark - SlideViewControllerSegue Class

@implementation SlideViewControllerSegue  // DEPRECATED
